...
(function () {
  ...
  function scrapeProduct() {
    ...
    return {
      ...
      bsrPrimaryCategory: parseBsrCategory(pairs),  // category of the primary "#N in <Cat>" — picks the funnel threshold
  ...
  // The CATEGORY of the PRIMARY Best Sellers Rank — "#1,234 in Beauty & Personal
  // Care" -> "Beauty & Personal Care". Picks the category-specific funnel
  // threshold. Empty string if no BSR line is present.
  function parseBsrCategory(pairs) {
    let text = '';
    const p = pairs.find(x => /best\s*sellers?\s*rank/i.test(x.label));
    if (p) text = p.value;
    if (!text) {
      const el = Array.from(document.querySelectorAll('li, tr, td, span, div'))
        .find(e => /best\s*sellers?\s*rank/i.test(e.textContent || '') && /#\s*[\d,]+\s+in\s+/i.test(e.textContent || ''));
      if (el) text = el.textContent || '';
    }
    if (!text) {
      const bt = document.body?.innerText || '';
    ...
    if (!text) return '';
    // First "#N in <Category>" — stop at "(", "#", or end (drops "(See Top 100)").
    const m = text.match(/#\s*[\d,]+\s+in\s+([^#(]+?)(?:\s*\(|\s*#|$)/i);
    return m ? clip(m[1].trim(), 80) : '';
  }
